...
Inputs come from the orchestration steps: the `tool` components (each with `plugpass_id`, `server_name`, `tool_name`, `is_free`, `gating_change_since_last_implement`, `template_stale`, and — for paired tools — `entitlement` (the pair's shared entitlement, identity only) + `operation`), the connector directive for this run (`connector`, `connector_change_since_last_implement`, `previous_connector`), the plugin-level values (`{PluginName}`, `{plugin-plugpass-id}`, `entitlement_api_origin`, `paywall_script_url`, `plugpass_issuer`, `plugpass_jwks_url`, `owned_servers`, `server_scaffolding_template_stale`), and the resolved `server_name → absolute source-directory path` map.
...
- Paired-tool add side → `feature_id: "{tool plugpass-component-id}"` — its OWN `tool_`-prefixed id, exactly like a solo tool. Every gated artifact bakes its own component's id; the `entitlement` subfield is identity, never a `feature_id`. Always `check_remaining`, passing the user's `current_count`. The remove side makes no Entitlement API call at all (identity only).
